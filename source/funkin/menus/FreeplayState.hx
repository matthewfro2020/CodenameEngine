...
class FreeplayState extends MusicBeatState
{
	...
	#if PRELOAD_ALL
	/**
	 * How much time a song stays selected until it botplays.
	 */
	public var timeUntilbotplay:Float = 1;
	/**
	 * Whenever the song botplays when hovered over.
	 */
	public var disablebotplay:Bool = false;
	/**
	 * Whenever the botplayed song gets async loaded.
	 */
	public var disableAsyncLoading:Bool = #if desktop false #else true #end;
	/**
	 * Time elapsed since last botplay. If this time exceeds `timeUntilbotplay`, the currently selected song will play.
	 */
	public var botplayElapsed:Float = 0;
	/**
	 ...
	 */
	public var botplayShouldPlay:Bool = true;
	#end
	...
	override function update(elapsed:Float)
	{
		...
		botplayElapsed += elapsed;
		if (!disablebotplay && !songInstPlaying && (botplayElapsed > timeUntilbotplay)) {
			if (curPlayingInst != (curPlayingInst = Paths.inst(curSong.name, curDifficulties[curDifficulty], curSong.instSuffix))) {
				...
				if (!streamed) {
					var huh:Void->Void = function() {
						...
						if (sound != null && botplayShouldPlay) {
							FlxG.sound.playMusic(sound, 0);
							Conductor.changeBPM(curSong.bpm, curSong.beatsPerMeasure, curSong.stepsPerBeat);
						}
					}
