...
// Amounts you want to give per hit
var botScoreGain:Int = 350;     // same as a "Sick" rating
var botHealthGain:Float = 0.02; // small health boost

function onPlayerHit(note) {
    // If botplay is active, give score + health
    if (curBotplay) {
        note.rating = "sick";
        PlayState.instance.songScore += botScoreGain;
        PlayState.instance.health += botHealthGain;
    }
}
function onPlayerMiss(note) {
    if (curBotplay) {
        return; // ignore misses entirely
    }
}